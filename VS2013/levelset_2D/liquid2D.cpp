...
static bool write_frame() {
	...
	sprintf_s( name, "frame_%d.bmp", counter++ );
...
static void addGravity() {
	OPENMP_FOR FOR_EVERY_Y_FLOW(gn) {
		if( j>0 && j<gn-1 && (A[i][j]<0.0f || A[i][j-1]<0.0f)) u[1][i][j] += -DT*GRAVITY;
		else u[1][i][j] = 0.0;
	} END_FOR;
	
	OPENMP_FOR FOR_EVERY_X_FLOW(gn) {
		if( i>0 && i<gn-1 && (A[i][j]<0.0f || A[i-1][j]<0.0f)) u[0][i][j] += 0.0f;
		else u[0][i][j] = 0.0;
	} END_FOR;
}
	...
	OPENMP_FOR FOR_EVERY_X_FLOW(gn) {
		if( i>0 && i<gn ) {
			...
			if( subcell && A[i][j] * A[i-1][j] < 0.0 ) {
				pf = A[i][j] < 0.0 ? p[i][j] : A[i][j]/fmin(0.0001f,A[i-1][j])*p[i-1][j];
				pb = A[i-1][j] < 0.0 ? p[i-1][j] : A[i-1][j]/fmin(0.0001f,A[i][j])*p[i][j];
			}
			u[0][i][j] -= (pf-pb)/h;
		}
	} END_FOR;
	...
	OPENMP_FOR FOR_EVERY_Y_FLOW(gn) {
		if( j>0 && j<gn ) {
			...
			if( subcell && A[i][j] * A[i][j-1] < 0.0 ) {
				pf = A[i][j] < 0.0 ? p[i][j] : A[i][j]/fmin(0.001f,A[i][j-1])*p[i][j-1];
				pb = A[i][j-1] < 0.0 ? p[i][j-1] : A[i][j-1]/fmin(0.00001f,A[i][j])*p[i][j];
			}
			u[1][i][j] -= (pf-pb)/h;
		}
	} END_FOR;
...
static void flow( float x, float y, float &uu, float &vv, float &dt ) {
	x = (gn-1)*fmin(1.0f,fmax(0.0f,x))+0.5f;
	y = (gn-1)*fmin(1.0f,fmax(0.0f,y))+0.5f;
	int i = x;
	int j = y;
...
void liquid2D::mouse( float x, float y, int state ) {
	...
	if( state == 0 && !moved ) {
		...
		float h = 0.1f/gn;
		x = fmin(1.0f-h,fmax(h,x));
		y = fmin(1.0f-h,fmax(h,y));
		FOR_EVERY_CELL(gn) {
			float qx = i/(float)gn;
			float qy = j/(float)gn;
...
void liquid2D::motion( float x, float y, float dx, float dy ) {
	...
	mousep[0] = x;
	mousep[1] = y;
	x = gn*fmin(1.0f-h,fmax(h,x));
	y = gn*fmin(1.0f-h,fmax(h,y));
	int i = int(x);
	int j = int(y);
	float s = 500.0f;
	u[0][i][j] += s*dx;
